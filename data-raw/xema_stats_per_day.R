...
xema_stats_per_day <- merge(xema_t_per_day, xema_rh_per_day, by=c("date_utc","station_code"))
xema_stats_per_day <- merge(xema_stats_per_day, xema_ws2_per_day, by=c("date_utc","station_code"))

columns <- names(xema_stats_per_day)
numeric_columns <- columns[!columns %in% c("date_utc", "station_code")]
xema_stats_per_day[numeric_columns] <- sapply(xema_stats_per_day[numeric_columns],as.numeric)
